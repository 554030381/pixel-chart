...
import {mapValueToColor, bilinearInterpolation} from '../utils/common.js'
import * as dhc from 'd3-horizon-chart'

const NAME_SPACE = 'everydayHorizon'

const FILE_NAME = 'Sisley法国希思黎2024-03-23 14_59_44.xls'
const SHEET_NAME = '抖音1天数据(数据集)'
...
// 将得到的二进制转化一下
  ...
  // 最后把数据转成json格式的
  let worksheet = workbook.Sheets[SHEET_NAME]; //这里是表格的名字,这里取第一个表格,1就是第二个表格数据
  ...
  let fieldObj = fields.reduce((obj, field, i) => {
    ...
    obj[field].min = Math.min(...fieldValues[i])
    obj[field].max = Math.max(...fieldValues[i])
  ...
  const container = d3.select(`#${NAME_SPACE}-horizon-chart`)
...
<template>
  <h3>每天</h3>
  <div id="everydayHorizon-horizon-chart"></div>
</template>

<style lang="scss" scoped>
#adHorizon-horizon-chart {
  width: 100%;
}
